...
class Pipeline:
    def __init__(self,  ratio, root, language, loop):
        self.ratio = ratio
        self.root = root
    ...
    def split_data(self, loop):
        ...
        train = pd.read_pickle(data_path + "wo_{}_{}_train.pkl".format(test_id1, test_id2))
        for i in range(1, loop+1):
            samples = pd.read_pickle(data_path + "uncertain_{}_{}_{}_{}.pkl".format(test_id1, test_id2, K, i))
            # samples = samples.sample(n=20000, replace=True)
            train = train.append(samples, ignore_index=True)

        # dev = data.iloc[train_split:val_split]
        dev = pd.read_pickle(data_path + "{}_{}_train.pkl".format(test_id1, test_id2))

        # test = data.iloc[val_split:]
        test = pd.read_pickle(data_path + "{}_{}_test.pkl".format(test_id1, test_id2))
...
def train(lang, loop, K):
    ...
    for t in range(1, categories+1):
        ...
        for epoch in range(EPOCHS):
            ...
            if best_res < f1:
                best_res = f1
                torch.save(model.state_dict(), "uncertain_{}_{}_{}.pt".format(K, test_id1, test_id2))
    ...
    df.to_pickle("data/c/uncertain_{}_{}_{}_{}.pkl".format(test_id1, test_id2, K, loop))
    print(df.head())
...
# train_id1, train_id2 = 1, 15
test_id1, test_id2 = 46, 60
K = 500
LOOP = 16
F1 = []
...
for loop in range(LOOP+1):
    ...
    ppl = Pipeline('3:1:1', 'data/', str(args.lang), loop)
    ppl.run()
    f1 = train(args.lang, loop+1, K)
    F1.append(f1)
